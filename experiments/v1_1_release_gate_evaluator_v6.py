...
def sealed_paths(content: str) -> set[str]:
    ...
    paths: set[str] = set()
    for item in raw:
        value = normalize(item.strip().strip("`*_ "))
...
def is_tooling_metadata(path: str) -> bool:
    name = Path(path).name
    return name in TOOLING_METADATA_NAMES or name.startswith("requirements")
...
def main() -> int:
    ...
    if args.case == "risk" and not (1 <= reviewer_count <= 2):
        failures.append(f"RISK requires 1..2 post-change Reviewer passes, got {reviewer_count}")
    if premium_count:
    ...
    for index, row in enumerate(rows):
        ...
        for request in tool_requests(row):
            ...
            if name == "task" and str(values.get("agent_type") or "").endswith("luna-reviewer"):
                reviewer_calls.append((index, str(values.get("prompt") or "")))

    for index, prompt in reviewer_calls:
        if first_mutation is None or index <= first_mutation:
            failures.append("Reviewer ran before a completed mutation existed")
    ...
    if args.case in {"tiny", "local"}:
        ...
        locator_count = 0
        semantic_reads: set[str] = set()
        tooling_reads: set[str] = set()
        for row in rows[:stop]:
            ...
            for request in tool_requests(row):
                ...
                if name in {"rg", "glob"}:
                    ...
                    elif is_tooling_metadata(path):
                        tooling_reads.add(path)
                    else:
                        semantic_reads.add(path)
        ...
        if locator_count > 3:
            failures.append(f"bounded locator budget exceeded: {locator_count} > 3")
        if len(semantic_reads) > 3:
            failures.append(f"bounded semantic read set exceeded: {len(semantic_reads)} > 3 ({sorted(semantic_reads)})")
        if len(tooling_reads) > 1:
            failures.append(f"bounded tooling metadata reads exceeded: {len(tooling_reads)} > 1 ({sorted(tooling_reads)})")
