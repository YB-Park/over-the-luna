#!/usr/bin/env python3
"""Validate the v1.1 ambient-tools VS Code Gate A candidate.

This branch is an integration candidate, not a release. The manifest must stay
at 1.0.0 until the authenticated real-VS-Code gate is complete.
"""
...
LUNA_IDS = EXPECTED_AGENT_IDS - {"premium-review"}
STRICT_TOOLS = {
    "luna-planner": set(),
...
def main() -> int:
    ...
    if actual_ids != EXPECTED_AGENT_IDS:
        errors.append(
            "agents/: ambient Gate A layout mismatch; "
    ...
    if premium:
        ...
        need(errors, path, body, "human-selected premium review", "NEEDS_EXTERNAL_VERIFICATION", "Do not recommend or invoke another premium model", "One Premium Review is the v1.1 visible premium decision")
        if fm.get("handoffs"):
    ...
    if errors:
        print("Over the Luna v1.1 ambient Gate A validation FAILED:\n")
        for error in errors:
            print(f"- {error}")
        return 1

    print(
        "Over the Luna v1.1 ambient Gate A validation passed: "
        f"{len(files)} agents, manifest v{plugin['version']}, ambient Main tools, instruction-sealed Council, one human Premium Review"
    )
    return 0
